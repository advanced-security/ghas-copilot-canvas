...
const DIAGNOSTIC_HEADERS = [
  ...
  ["X-GitHub-Api-Version-Selected", "x-github-api-version-selected"],
...
export function selectDiagnosticHeaders(headers) {
  ...
  const selected = {};
  for (const [displayName, lookupName] of DIAGNOSTIC_HEADERS) {
    const value = headers instanceof Headers ? headers.get(lookupName) : normalized[lookupName];
    if (value != null && value !== "") selected[displayName] = value;
  }
  return selected;
}
